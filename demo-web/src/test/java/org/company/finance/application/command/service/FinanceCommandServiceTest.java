...
class FinanceCommandServiceTest {
    ...
    void shouldSaveRecordWhenCategoryMatchesExpenseType() {
        ...
        when(queryCategoryRepository.findById(1)).thenReturn(buildCategoryEntity(1));
    ...
    void shouldRejectRecordWhenCategoryTypeDoesNotMatch() {
        CreateExpenseRecordRequest request = buildCreateRequest();
        CategoryEntity categoryEntity = buildCategoryEntity(2);
        when(queryCategoryRepository.findById(1)).thenReturn(categoryEntity);
    ...
    private CategoryEntity buildCategoryEntity(Integer expenseType) {
        CategoryEntity categoryEntity = new CategoryEntity();
        categoryEntity.setId(1);
        categoryEntity.setCategoryName("餐饮");
        categoryEntity.setParentId(0);
        categoryEntity.setExpenseType(expenseType);
        categoryEntity.setDelFlag(0);
