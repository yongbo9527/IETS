...
import org.company.finance.tally.application.query.model.RecordVO;
...
import org.company.finance.tally.interfaces.rest.response.DynamicTableResponse;
import org.company.finance.tally.interfaces.rest.response.ExpenseRecordResponse;
import org.company.finance.tally.interfaces.rest.response.TableDetailHeader;
...
public class FinanceQueryService {
    ...
    public Page<ExpenseRecordResponse> listDailyRecords(QueryBalanceVO vo) {
        Page<ExpenseRecordQueryModel> expenseRecordEntityPage = queryRecordRepository.findByUserIdAndDateRange(null, vo);
    ...
    public DynamicTableResponse getBalanceTable(QueryBalanceVO vo) {
        ...
        // 1. 校验分类是否存在
        List<CategoryResponse> categories = queryCategoryRepository.findActiveOrderByAsc();
    ...
    private LinkedList<TableDetailHeader> buildHeaders(List<CategoryResponse> categories) {
        LinkedList<TableDetailHeader> headers = new LinkedList<>();
        headers.add(new TableDetailHeader("expenseDate", "日期", null));
    ...
    private Page<LinkedHashMap<String, Object>> buildCompactBalanceTableRows(QueryBalanceVO vo) {
        ...
        Page<ExpenseRecordQueryModel> datePage = queryRecordRepository.findPage(vo, page.getCurrent(), page.getSize());
        List<String> expenseDates = datePage.getRecords().stream()
                .map(ExpenseRecordQueryModel::getExpenseDate)
                .collect(Collectors.toList());
        ...
        // 2. 查询这些日期下的所有明细记录
        List<ExpenseRecordQueryModel> allRecords = queryRecordRepository.findList(expenseDates);

        // 3. 按 categoryId + expenseDate 分组并累加金额
        Map<String, ExpenseRecordQueryModel> groupedMap = allRecords.stream()
    ...
    public Page<LinkedHashMap<String, Object>> queryGroupedRecords(QueryBalanceVO vo, List<CategoryResponse> categories) {

        // 1. 先查出所有涉及的日期（分页）
        Page<String> datePage = new Page<>(vo.getCurrent(), vo.getPageSize());
        Page<ExpenseRecordQueryModel> pageResult = queryRecordRepository.findDistinctDates(vo, vo.getCurrent(), vo.getPageSize());
        ...
        // 2. 查出这些日期下的所有记录
        List<ExpenseRecordQueryModel> records = queryRecordRepository.findByDates(datePage.getRecords());
    ...
    private CategoryNode toCategoryNode(CategoryResponse entity) {
        return new CategoryNode(entity.getId(), entity.getCategoryName(), entity.getParentId());
    }

    private ExpenseRecordResponse toExpenseRecordResponse(ExpenseRecordQueryModel entity) {
